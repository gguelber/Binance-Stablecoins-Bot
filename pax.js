...
function setBuySell() {
	try {
		binance.prices('PAXUSDT', (error, ticker) => {
			if (error) return console.error(error);
			console.log('==========================================');
			ticker.PAXUSDT <= config.MIN_BUY_PRICE
				? console.log(`O preço já está favorável, vamos lucrar`)
				: console.log(`O preço do PAX ainda está acima do mínimo de ${config.MIN_BUY_PRICE} configurado.`);

			if (ticker.PAXUSDT <= config.MIN_BUY_PRICE && availableUsdt > 20) {
				try {
					binance.buy('PAXUSDT', ((availableUsdt - 0.1) / ticker.PAXUSDT).toFixed(2), ticker.PAXUSDT);
					buyPrice = ticker.PAXUSDT;
					sellPrice = buyPrice + config.SELL_MARGIN;
					console.log('Comprado a: ' + ticker.PAXUSDT);
				} catch (e) {
					throw e;
				}
			}

			if (ticker.PAXUSDT > config.MIN_BUY_PRICE && availableTusd > 20) {
				try {
